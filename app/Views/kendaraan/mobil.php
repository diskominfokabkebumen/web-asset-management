            ...
            <?php if (!empty($kendaraan)) : ?>
                <?php foreach ($kendaraan as $key => $row) : ?>
                    <tr>
                        ...
                        <td><?= esc($row['warna']) ?></td>
                        <td>
                            <a href="<?= site_url('kendaraan/editmobil/'. $row['id']) ?>" class="btn btn-warning btn-sm">Edit</a>
                            <a href="<?= site_url('kendaraan/riwayatmobil/'. $row['id']) ?>" class="btn btn-info btn-sm">Riwayat</a>
                            <form action="<?= site_url('kendaraan/deletemobil/'. $row['id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                <?= csrf_field() ?>
