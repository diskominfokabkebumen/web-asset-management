            ...
            <?php if (!empty($kendaraan)) : ?>
                <?php foreach ($kendaraan as $key => $row) : ?>
                    <tr>
                        ...
                        <td><?= esc($row['warna']) ?></td>
                        <td>
                           <a href ="<?= site_url('kendaraan/editmotor/'. $row['id']) ?>" class="btn btn-warning btn-sm">Edit</a>
                            <form action="<?= site_url('kendaraan/deletemotor/'. $row['id']) ?>" method="post" class="d-inline">
                                <?= csrf_field() ?>
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                <a href="<?= site_url('kendaraan/riwayatmotor/'. $row['id']) ?>" class="btn btn-info btn-sm">Riwayat</a>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
