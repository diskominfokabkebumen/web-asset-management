...
<div class="my-3 p-3 bg-body rounded shadow-sm">
    <div class="d-flex justify-content-between border-bottom py-2">
        ...
        <a href="<?= site_url('auth/users/create') ?>" class="btn btn-primary">Tambah User</a>
    </div>
                ...
                <?php if (!empty($users)) : ?>
                    <?php foreach ($users as $key => $u) : ?>
                        <tr>
                            ...
                            <td><?= esc($u['role']) ?></td>
                            <td>
                                <form action="<?= site_url('auth/users/delete/' . $u['id']) ?>" method="post" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus user ini?');">
                                    <?= csrf_field() ?>
